c$Id: scatter.f,v 1.30 2007/05/23 14:28:51 bleicher Exp $
...
c     prepare output to decay file
         ...
         sqrts=fmass(ind1)
         trmass2= px(ind1)**2+py(ind1)**2+fmass(ind1)**2 !### kw ###
         betax=px(ind1)/sqrt(trmass2+pz(ind1)**2)        !### kw ###
         betay=py(ind1)/sqrt(trmass2+pz(ind1)**2)        !### kw ###
         betaz=pz(ind1)/sqrt(trmass2+pz(ind1)**2)        !### kw ###
         p0nn=p0(ind1)
         pxnn=px(ind1)
         pynn=py(ind1)
         pznn=pz(ind1)
...
c... prepare exit channel
      ...
      call printReaction(id1,id2,0,1,1)                     !###### kw ######
...
C####C##1#########2#########3#########4#########5#########6#########7##
      ...
      if(BEX**2+BEY**2+BEZ**2.ge.1d0)then            !### kw ###  
        vvv=sqrt(BEX**2+BEY**2+BEZ**2)               !### kw ###
        print*,'warning in rotbos: v = ',vvv ,' >= 1'!### kw ###
        print*,'beta=',BEX,BEY,BEZ                   !### kw ###
        BEX=BEX/vvv*0.999999999d0                    !### kw ###
        BEY=BEY/vvv*0.999999999d0                    !### kw ###
        BEZ=BEZ/vvv*0.999999999d0                    !### kw ###
      endif                                          !### kw ###
