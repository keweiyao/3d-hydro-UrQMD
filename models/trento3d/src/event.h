...
//>>MODIFICATION!-------added by Weiyao-----------
  // Alias for a two-dimensional thickness grid.
  using Grid_3d = boost::multi_array<double, 3>;
...
//<<MODIFICATION!-------added by Weiyao-----------
  ...
  const Grid_3d& reduced_thickness_grid_3d() const
  { return TR_3d_; }
  const Grid_1d& reduced_rapidity_profile() const
    { return dNdy_;}
  const Grid&  get_event_plane() const
    { return event_plane_list_; }
 ...
 private:
  ...
  std::vector<double> cumulent_reconstruction(double ta, double tb);
  ...
  const double norm_;
  const double skew_coeff_, kurtosis_, K_, mean_;
  const double sigma_;
  ...
  const double xymax_;
//>>MODIFICATION!-------added by Weiyao-----------
  /// half width of pseudorapidity range
  const double eta_max_;
  const int eta_steps_;
  /// switch for 3d IC calculation
  const bool switch_3d_;
  const bool out_3d_;
//<<MODIFICATION!-------added by Weiyao-----------

  /// Nuclear thickness grids TA, TB and reduced thickness grid TR.
  Grid TA_, TB_, TR_, event_plane_list_;
//>>MODIFICATION!-------added by Weiyao-----------
