...
void Cell::correctQideal(EoS *eos, double tau)
{
    ...
    const double tau_e_p_gamma2 = tau*(e+p)*gamma2;
    const double tau_gamma = tau*sqrt(gamma2);
    Q[T_] = tau*(e+p*v2)*gamma2 ;
    Q[X_] = tau_e_p_gamma2*vx ;
    Q[Y_] = tau_e_p_gamma2*vy ;
    Q[Z_] = tau_e_p_gamma2*vz ;
    Q[NB_] = tau_gamma*nb ;
    Q[NQ_] = tau_gamma*nq ;
    Q[NS_] = tau_gamma*ns ;
}
...
void Cell::setPrimVar(EoS *eos, double tau, double _e, double _nb, double _nq, double _ns, double _vx, double _vy, double _vz)
{
  const double v2 = _vx*_vx + _vy*_vy + _vz*_vz;
	...
	const double tau_e_p_gamma2 = tau*(_e+p)*gamma2;
	const double tau_gamma = tau*sqrt(gamma2);
	Q[T_] = tau*(_e+p*v2)*gamma2 ;
	Q[X_] = tau_e_p_gamma2*_vx ;
	Q[Y_] = tau_e_p_gamma2*_vy ;
	Q[Z_] = tau_e_p_gamma2*_vz ;
	Q[NB_] = tau_gamma*_nb ;
	Q[NQ_] = tau_gamma*_nq ;
	Q[NS_] = tau_gamma*_ns ;
	if(!(Q[NB_]<0. || Q[NB_]>=0.)){
